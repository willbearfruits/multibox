...
# keep any fingerprints authorized in a previous run / by `multibox pair`
auth=$(awk '/^\[authorized_fingerprints\]/{f=1;next} /^\[/{f=0} f&&NF' \
  ~/.config/lan-mouse/config.toml 2>/dev/null || true)
{
  echo "# generated by multibox install.sh — edit machines.json + re-run instead"
  echo 'port = 4242'
  echo 'release_bind = ["KeyLeftCtrl", "KeyLeftShift", "KeyLeftAlt", "KeyRightCtrl"]'
  if [ "$MY_ROLE" = desktop ]; then
    jq -r --arg ip "$MY_IP" '.machines[] | select(.ip != $ip) |
      "\n[[clients]]\nposition = \"\(.role)\"\nhostname = \"\(.name)\"\nips = [\"\(.ip)\"]\nactivate_on_startup = true"' "$CONFIG"
  else
    OPP=$([ "$MY_ROLE" = left ] && echo right || echo left)
...
systemctl --user try-restart clipsync.service lan-mouse.service
...
if [ "$MY_ROLE" != desktop ]; then
  ...
  cat > ~/.local/bin/desktop-monitor <<EOF
#!/bin/bash
exec vncviewer -FullScreen $DESKTOP_IP:$VNC_PORT
EOF
