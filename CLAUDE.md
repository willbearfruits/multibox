...
- **`bin/multibox`** is the fleet CLI (status/setup/apply/send/send-window/kvm/pause/resume/doctor/drop/extend/pair/pairing-info). `kvm on|off` is the fleet-wide mouse/keyboard-sharing master switch: it writes `options.kvm`, flips the local lan-mouse service, and peers adopt it via `apply --from-drop` (apply enables/disables lan-mouse from that option). `pause` touches `$XDG_RUNTIME_DIR/clipsync.paused` rather than stopping the service — the clipsync daemon keeps serving `/file` and `/monitor` while paused (push no-ops; `/clip` answers 503 so senders retry later). The Omarchy bar widget (`plugin/BarWidget.qml`) is a thin view over `multibox status --json` and shells out to the same CLI for actions; the GTK4/libadwaita setup window (`setup/multibox-setup.py`) edits machines.json and runs `multibox apply`.
- **Extra-monitor pipeline** (`multibox extend start <side>`, desktop only): headless Hyprland output positioned past that edge → `wayvnc -g -f <fps>` on port 5901/5902 (GPU capture with software fallback, view-only unless `options.extend.input`) → a `POST /monitor {"action":"start"}` to that side's laptop makes its clipsync daemon launch the fullscreen `desktop-monitor` viewer. The viewer has two flavors chosen by `options.extend.h264` at apply time: **wlvncc** speaking `open-h264` (desktop GPU encodes video, laptop iGPU decodes — the good one) or TigerVNC with pinned Tight/low-compression as fallback. wlvncc is NOT packaged: it's built from source and shipped to the laptops' `~/.local/bin/wlvncc` with a bundled `libaml` in `~/.local/lib` — not a repo artifact, rebuild via github.com/any1/{aml,wlvncc} + meson-from-checkout if a machine is reinstalled. The viewer runs as its own transient unit (`systemd-run --user --unit desktop-monitor`), NOT as a clipsync child — restarting clipsync kills its whole cgroup, and the viewer must survive that. `multibox extend start <side>` on an already-extended side just re-triggers the viewer, which is how you relaunch a closed one. While a side is extended its lan-mouse client is paused (same edge, two owners) — durably: `gen_lm_config` writes `activate_on_startup = false` for extended sides and extend start/stop regenerates the config, so restarts of lan-mouse can't re-arm a borrowed edge (a live `lan-mouse cli deactivate` alone does not survive restarts). `multibox send-window <side>` = `hyprctl dispatch movewindow mon:<headless>` from the extend state file. Windows can only be *displayed* remotely, never migrated — don't promise laptop→desktop window moves.
